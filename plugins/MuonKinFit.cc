...
MuonKinFit::MuonKinFit(const edm::ParameterSet& iConfig) :
  srcCands_(consumes<edm::View<pat::Muon>>(iConfig.getParameter<edm::InputTag>("srcCands")))
{
...
public:
  ...
  DimuonKinematicFitResult(const TLorentzVector& _mu1_tlv, const TLorentzVector& _mu2_tlv) :
    mu1_tlv(_mu1_tlv),
    mu2_tlv(_mu2_tlv) {}
};

DimuonKinematicFitResult dimuon_kinematic_fit(const pat::Muon& mu1, const pat::Muon& mu2) {
  ...
  std::cout << "Kin Fitted muons 1 :" << mu1_tlv.Pt() << " err " << mu1_ptErr_kinfit << "  -- Pat muons : " << muonsWithTrack.at(0).pt() << std::endl;
  std::cout << "Kin Fitted muons 2 :" << mu2_tlv.Pt() << " err " << mu2_ptErr_kinfit << "  -- Pat muons : " << muonsWithTrack.at(1).pt() << std::endl;
...
void
MuonKinFit::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  if (muonsSelected.size() >= 2) {
    const auto kinfit_res = dimuon_kinematic_fit(muonsSelected.at(0), muonsSelected.at(1));
    muonsSelected.at(0).addUserFloat("pt_dimuon_kinfit", kinfit_res.mu1_tlv.Pt());
    muonsSelected.at(1).addUserFloat("pt_dimuon_kinfit", kinfit_res.mu2_tlv.Pt());

    if (muonsSelected.size() >= 3) {
      std::cerr << "Warning: kinematic fit only calculated for the first two muons, but we had " << muonsSelected.size() << std::endl;
    }
  }
...
void
MuonKinFit::beginStream(edm::StreamID)
{
}

// ------------ method called once each stream after processing all runs, lumis and events  ------------
void
MuonKinFit::endStream() {
}

// ------------ method called when starting to processes a run  ------------
/*
void
MuonKinFit::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void
MuonKinFit::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void
MuonKinFit::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void
MuonKinFit::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
MuonKinFit::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
